...
set_costs_all[set_costs_all == 0] <- 0.000001
...
network_analysis_long_expansion <- network_analysis_long %>%
  ...
  mutate(upstream_nccag = case_when(gage_location == "downstream" & nccag == 1 ~ 0.5,
         ...
         on_nccag = case_when(gage_location == "on comid" & nccag == 1 ~ 0.5,
                              TRUE ~ 0),
         upstream_nid_dam_dor = case_when(gage_location == "downstream" & nid_dam == 1 ~ 0.5,
                                          TRUE ~ 0),
         downstream_nid_dam_dor = case_when(gage_location == "upstream" & nid_dam == 1 ~ 0.5,
                                            TRUE ~ 0),
         on_nid_dam_dor = case_when(gage_location == "on comid" & nid_dam == 1 ~ 0.5,
                                    TRUE ~ 0))
...
# value but will get in the way of set-cover weighting; essentially, there is
# no "cost" to selecting these sets
set_costs_expansion[set_costs_expansion == 0] <- 0.000001



# Compute the value of each flowline in association with its reconfigure gage(s) ----

# mutate on value-relevant columns for reconfigure network
network_analysis_long_reconfig <- network_analysis_long %>%
...
# value but will get in the way of set-cover weighting; essentially, there is
# no "cost" to selecting these sets
set_costs_reconfig[set_costs_reconfig == 0] <- 0.000001